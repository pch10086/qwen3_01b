#!/usr/bin/env python3
"""Generation-aware attention collapse analysis for self-trained qwen3_01b checkpoints.

V1 measured attention from the final prompt token. This script measures attention
from the first generated answer token by:

1. running a greedy prefill on the prompt to choose the first generated token;
2. running generation for a short answer string for scoring;
3. running one analysis-only full forward on prompt + first token, collecting
   attention summaries at the generated-token position.

The model/training implementation is not modified; attention modules are wrapped
at runtime for this analysis process only.
"""
...
class AttentionSummaryCollector:
    ...
    def begin(self, *, evidence_start: int, evidence_end: int, query_position: int) -> None:
        self.evidence_start = evidence_start
        self.evidence_end = evidence_end
        self.query_position = query_position
        self.rows.clear()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", default="analysis/attention_collapse/generation_probe_v2")
    parser.add_argument("--examples-jsonl", default=None, help="Optional existing examples.jsonl to reuse.")
    ...
    parser.add_argument("--max-examples", type=int, default=None, help="Limit examples after construction/loading.")
    parser.add_argument("--methods", nargs="*", default=None, help="Optional method name filter, e.g. rope_none rope_linear.")
    parser.add_argument("--write-figures", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--dtype-autocast", action=argparse.BooleanOptionalAction, default=True)
...
def build_examples(tokenizer: Any, args: argparse.Namespace) -> list[ProbeExample]:
    if args.examples_jsonl:
        ...
        with path.open("r", encoding="utf-8") as f:
            for line in f:
                if line.strip():
                    rows.append(ProbeExample(**json.loads(line)))
...
def make_attention_wrapper(original_forward: Any, layer_idx: int, collector: AttentionSummaryCollector):
    def wrapped(self: Any, x: torch.Tensor, mask: torch.Tensor | None, cos: torch.Tensor, sin: torch.Tensor, past_kv=None, use_cache=False, position_offset=0):
        if past_kv is None and not use_cache and x.shape[0] == 1 and collector.enabled and collector.query_position is not None:
            with torch.no_grad():
...
def summarize_attention_layer(
    *,
    ...
    if collector.query_position is None:
        return
    if collector.query_position >= num_tokens:
        raise ValueError(
            f"query_position={collector.query_position} is outside current sequence length {num_tokens}"
        )
    ...
    end_pos = int(collector.query_position)
    query_window = max(1, min(collector.query_window, end_pos + 1))
    query_positions = torch.arange(end_pos - query_window + 1, end_pos + 1, device=x.device)
    ...
    collector.add_layer_summary(layer=layer_idx, attn=attn, query_positions=query_positions, seq_len=num_tokens)
...
def maybe_autocast(device: torch.device, enabled: bool):
    return torch.amp.autocast("cuda", dtype=torch.bfloat16) if enabled and device.type == "cuda" and torch.cuda.is_bf16_supported() else nullcontext()
...
def sample_next_from_logits(logits: torch.Tensor, *, temperature: float, top_k: int | None) -> torch.Tensor:
...
def generate_ids(
    *,
    ...
    if idx.shape[1] > context_length:
        raise ValueError(f"Prompt has {idx.shape[1]} tokens, exceeding context_length={context_length}")
    for _ in range(max_new_tokens):
        with maybe_autocast(device, dtype_autocast):
            logits = model(idx[:, -context_length:])[:, -1, :]
        next_id = sample_next_from_logits(logits, temperature=temperature, top_k=top_k)
        idx = torch.cat([idx, next_id], dim=1)
    return idx[0, len(prompt_ids) :].detach().cpu().tolist()


@torch.inference_mode()
def collect_generated_token_attention(
    *,
    model: Any,
    prompt_ids: list[int],
    first_token_id: int,
    ...
    input_ids = torch.tensor([prompt_ids + [first_token_id]], dtype=torch.long, device=device)
    context_length = int(model.cfg["context_length"])
    if input_ids.shape[1] > context_length:
        raise ValueError(f"Prompt + first generated token has {input_ids.shape[1]} tokens, exceeding context_length={context_length}")
    collector.begin(
...
def score_prediction(generated_text: str, answer: str) -> dict[str, Any]:
    ...
    return {
        "exact_match": int(normalize_answer(first_line) == normalize_answer(answer)),
        "contains_answer": int(answer in generated_text),
        "first_number": first_number,
        "first_number_match": int(first_number == answer),
        "num_numbers": len(numbers),
        "format_error": int(first_number == ""),
    }
...
def aggregate_rows(rows: list[dict[str, Any]], group_keys: tuple[str, ...]) -> list[dict[str, Any]]:
    ...
    for key_values, group in sorted(groups.items(), key=lambda item: item[0]):
        ...
        record["n"] = len(group)
        for numeric_key in metric_keys():
        ...
        out.append(
            {
                ...
                "position_ratio": position,
                "n": n,
                "exact_match": sum(int(r["exact_match"]) for r in group) / n,
                "contains_answer": sum(int(r["contains_answer"]) for r in group) / n,
                "first_number_match": sum(int(r["first_number_match"]) for r in group) / n,
                "format_error_rate": sum(int(r["format_error"]) for r in group) / n,
                "mean_latency_sec": sum(float(r["latency_sec"]) for r in group) / n,
            }
        )
    return out


def aggregate_correct_vs_wrong(metric_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return aggregate_rows(metric_rows, group_keys=("method", "context_length_target", "position_ratio", "first_number_match"))
...
def write_accuracy_by_position(pred_summary: list[dict[str, Any]], output_dir: Path) -> None:
    ...
    for length, rows in sorted(by_length.items()):
        ...
        values = {(str(row["method"]), float(row["position_ratio"])): float(row["first_number_match"]) for row in rows}
        write_line_plot(
            output_dir / f"accuracy_by_position_len{length}.svg",
            title=f"Generation Accuracy by Position ({length} tokens)",
            subtitle="Metric: first generated number equals target answer",
            y_label="Accuracy",
            methods=methods,
            positions=positions,
            values=values,
...
def write_evidence_by_position(method_summary: list[dict[str, Any]], output_dir: Path) -> None:
    by_length: dict[int, list[dict[str, Any]]] = defaultdict(list)
    ...
    for length, rows in sorted(by_length.items()):
        ...
        write_line_plot(
            output_dir / f"generation_evidence_mass_by_position_len{length}.svg",
            title=f"Generated-Token Evidence Mass by Position ({length} tokens)",
            subtitle="Mean attention mass from first generated token to evidence span",
...
def write_line_plot(
    ...
    *,
    ...
    margin = {"left": 74, "right": 190, "top": 70, "bottom": 72}
    plot_w = width - margin["left"] - margin["right"]
    plot_h = height - margin["top"] - margin["bottom"]

    max_v = max([v for v in values.values() if not math.isnan(v)] + [1.0])
    y_max = 1.0 if max_v <= 1.0 else max_v

    def x_scale(pos: float) -> float:
        return margin["left"] + pos * plot_w

    def y_scale(value: float) -> float:
        return margin["top"] + (1.0 - max(0.0, min(y_max, value)) / y_max) * plot_h
    ...
    write_text_svg(path, width, height, body)


def write_layer_head_heatmaps(summary_rows: list[dict[str, Any]], output_dir: Path) -> None:
    metrics = {
        "generation_entropy_collapse": ("entropy_norm", lambda v: 1.0 - v, 0.0, 1.0, "1 - normalized entropy"),
        "generation_sink_mass": ("sink_mass_first16", lambda v: v, 0.0, 1.0, "Attention mass on first 16 tokens"),
        "generation_evidence_mass": ("evidence_mass", lambda v: v, 0.0, 1.0, "Attention mass on evidence span"),
    }
    ...
    for (method, length), rows in by_panel.items():
        ...
        for metric_name, (source_key, transform, lo, hi, title) in metrics.items():
            body: list[str] = [
                ...
                svg_text(width / 2, 52, "rows = layer, columns = head; query = first generated token", size=11, anchor="middle"),
            ]
...
def main() -> int:
    ...
    metrics_path = output_dir / "generation_attention_metrics.jsonl"
    ...
    write_json(
        ...
        | {
            ...
            "analysis_query": "first_generated_token",
        },
    )
    ...
    for method, checkpoint_path in checkpoints:
        ...
        model.eval()
        collector = AttentionSummaryCollector(evidence_start=None, evidence_end=None, query_window=args.query_window)
        ...
        with torch.inference_mode():
            for index, example in enumerate(examples, start=1):
                ...
                if not generated_ids:
                    raise RuntimeError("No tokens generated; set --max-new-tokens > 0")
                generated_text = tokenizer.decode(generated_ids)
                first_token_id = int(generated_ids[0])
                first_token_text = tokenizer.decode([first_token_id])
                scores = score_prediction(generated_text, example.answer)

                metric_rows = collect_generated_token_attention(
                    model=model,
                    prompt_ids=prompt_ids,
                ...
                pred_row = {
                    ...
                    "analysis_seq_len": len(prompt_ids) + 1,
                    "latency_sec": elapsed,
                    **scores,
                }
                    ...
                    row.update(
                        {
                            ...
                            "sample_index": example.sample_index,
                            "answer": example.answer,
                            "first_number_match": scores["first_number_match"],
                            "exact_match": scores["exact_match"],
                            "contains_answer": scores["contains_answer"],
                            "format_error": scores["format_error"],
                            "generated_text": generated_text,
                            "first_token_id": first_token_id,
                            "first_token_text": first_token_text,
                            "prompt_tokens": len(prompt_ids),
                ...
                print(
                    ...
                    f"tokens={len(prompt_ids)} answer={example.answer} first_number={scores['first_number'] or 'NA'} "
                    f"match={scores['first_number_match']} rows={len(metric_rows)} time={elapsed:.2f}s",
                    flush=True,
                )
    ...
    prediction_summary = aggregate_predictions(all_prediction_rows)
    layer_head_summary = aggregate_rows(
        all_metric_rows,
        group_keys=("method", "context_length_target", "position_ratio", "layer", "head"),
    )
    method_position_summary = aggregate_rows(
        all_metric_rows,
        group_keys=("method", "context_length_target", "position_ratio"),
    )
    correct_vs_wrong_summary = aggregate_correct_vs_wrong(all_metric_rows)

    write_csv(output_dir / "prediction_summary.csv", prediction_summary)
    write_csv(output_dir / "generation_layer_head_summary.csv", layer_head_summary)
    write_csv(output_dir / "generation_method_position_summary.csv", method_position_summary)
    write_csv(output_dir / "correct_vs_wrong_summary.csv", correct_vs_wrong_summary)

    if args.write_figures:
        figures_dir = output_dir / "figures"
        write_accuracy_by_position(prediction_summary, figures_dir)
        write_evidence_by_position(method_position_summary, figures_dir)
        write_layer_head_heatmaps(layer_head_summary, figures_dir)

    print(f"\nWrote examples: {output_dir / 'examples.jsonl'}")
    print(f"Wrote predictions: {predictions_path}")
    print(f"Wrote generation attention metrics: {metrics_path}")
    print(f"Wrote prediction summary: {output_dir / 'prediction_summary.csv'}")
    print(f"Wrote generation layer/head summary: {output_dir / 'generation_layer_head_summary.csv'}")
    print(f"Wrote generation method/position summary: {output_dir / 'generation_method_position_summary.csv'}")
    print(f"Wrote correct-vs-wrong summary: {output_dir / 'correct_vs_wrong_summary.csv'}")
    if args.write_figures:
