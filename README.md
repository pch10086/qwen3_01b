...
| `model.py` | `Qwen3Model` |
| `data.py` | 滑窗 `LMDataset`、token shard `TokenShardDataset`、随机 `SyntheticLMDataset`、DataLoader |
| `losses.py` | 下一词交叉熵 |
| `generate.py` | `generate`、长 prompt 截断 `trim_input_to_context` |
| `tokenizer_utils.py` | 从 `tokenizer.json` 加载分词（与 Qwen 词表一致时 `vocab_size=151936`） |
| `training.py` | `train`、`train_pretrain`、DDP/resume/checkpoint/JSONL 日志 |
| `config_utils.py` | checkpoint 里 `dtype` 等可序列化 config |
| `weights.py` | 可选：从 HuggingFace 键名灌权重（你若以后要对齐官方再开） |
| `cli_train.py` / `cli_pretrain.py` / `cli_infer.py` | 小语料训练、真实 token-manifest 预训练与推理 |
...
真实预训练入口是 `cli_pretrain.py`，输入应是预编码 token shard manifest，而不是原始 txt。manifest 示例：
